            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const res = await fetch(urls.ask, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
                        body: JSON.stringify({ body, author_name: name, token }),
                    });
                    if (res.ok) {
                        const data = await res.json();
                        document.getElementById('qbody').value = '';
                        const hint = document.getElementById('askHint');
                        hint.textContent = data.pending ? '✅ 已送出，待主辦審核後顯示。' : '✅ 已送出！';
                        await load();
                    } else {
                        document.getElementById('askHint').textContent = '送出失敗，請再試一次。';
                    }
